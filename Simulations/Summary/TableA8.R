...
my.table <- df  %>% mutate(n.sample = factor(n.sample), lambda = factor(lambda)) %>% 
    filter(n.sample %in% c(500, 1000) &lambda %in% c(0,1,5) & n.knots %in% c(0,5,10)  & censoring %in% c(2)) %>% 
  filter(!is.na(beta.y.est)) %>% 
  ...
  group_by(n.sample, n.knots, lambda) %>%
...
my.table.beta.y <- t(my.table)[c(1:3, 4:7),]
my.table.nt1 <- t(my.table)[c(1:3, 8:11),]
my.table.nt2 <- t(my.table)[c(1:3, 12:15),]
my.table.t1 <- t(my.table)[c(1:3, 16:19),]
my.table.t2 <- t(my.table)[c(1:3, 20:23),]
my.table.or1 <- t(my.table)[c(1:3, 24:27),]
my.table.or2 <- t(my.table)[c(1:3, 28:31),]

# Table A.8
my.table.beta.y %>% xtable
my.table.or1 %>% xtable()
my.table.or2 %>% xtable()
